...
async def get_price_intelligence(category: str = Query(..., min_length=1), _: dict = Depends(require_b2b)):
    ...
    try:
        ...
        prompt = (
            ...
            f'  "reason": "가격 판단 근거 2문장. 1문장: 현재 평균가와 시장 가격 위치 해석 (~으로 판단됩니다 어조, 수치 필수). 2문장: 성수기·계절성을 반영한 재고 확보 전략 (~을 확보하는 전략이 유효합니다 어조).",\n'
            f'  "strategy": "B2B 구매 전략 2문장. 1문장: 매입 방식과 리스크 관리 방향 (분할 매입/선매입 여부, ~가 권장됩니다 어조). 2문장: 권장 판매가 구간과 단계적 매입 전략 (~을 유지하는 것이 바람직합니다 어조).",\n'
