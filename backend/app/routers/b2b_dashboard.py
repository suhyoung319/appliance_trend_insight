...
async def get_b2b_dashboard(category: str = Query(..., min_length=1), period: str = "3m", _: dict = Depends(require_b2b)):
    ...
    try:
        ...
        prompt = (
            ...
            f'  "conclusion_lines": [\n'
            f'    "줄1 (관심도 흐름): 현재 검색 관심도 {current}와 {period} 성장률({growth:+}%) 기반으로 수요 방향을 1문장으로. 예: 관심도 {current} 수준으로 {period} 성장률 {growth:+}%를 기록하며 ~~.",\n'
            f'    "줄2 (브랜드·소비층): {top3} 브랜드 경쟁 구도와 {top_age} 핵심 소비층 특성 1문장. 예: {top3} 중심 경쟁 구도에서 {top_age}이 ~~.",\n'
            f'    "줄3 (연관 가전 수요): 위 데이터에서 도출한 연관 가전 제품 수요와 구매 패턴 1문장. 예: ~~와 함께 연관 가전 동반 수요가 ~~."\n'
            f'  ]\n'
            f'}}'
        )
