    ...
    return (
        <div className={s.cloud}>
            {keywords.map((kw, i) => {
                ...
                return (
                    <span key={kw} className={`${s.cloudTag} ${s[`cloudTag${size}`]}`}>
                        {kw}
                    </span>
                );
            })}
...
function DriverGroup({ title, items, type }) {
    if (!items || items.length === 0) {
        return (
            <div className={s.driverGroup}>
                <p className={s.driverTitle}>{title}</p>
                <div className={s.driverEmpty}>데이터 부족</div>
            </div>
        );
    }
    ...
    return (
        <div className={s.driverGroup}>
            <p className={s.driverTitle}>{title}</p>
            <div className={s.driverTags}>
                {items.map((kw) => (
                    <span key={`${type}-${kw}`} className={`${s.driverTag} ${s[`driverTag${type}`]}`}>
                        {kw}
                    </span>
                ))}
            </div>
...
function TrendDrivers({ drivers }) {
    return (
        <div className={s.driverWrap}>
            <DriverGroup title="상승 키워드" items={drivers?.rising} type="Rising" />
            <DriverGroup title="시즌성 키워드" items={drivers?.seasonal} type="Seasonal" />
            <DriverGroup title="하락 키워드" items={drivers?.falling} type="Falling" />
        </div>
    );
}

function PainPoints({ painPoints }) {
    const top = painPoints?.top ?? [];
    const suggestions = painPoints?.suggestions ?? [];

    return (
        <div className={s.painWrap}>
            <div className={s.painSection}>
                <p className={s.painTitle}>불만 키워드 TOP 5</p>
                {top.length === 0 ? (
                    <div className={s.driverEmpty}>데이터 부족</div>
                ) : (
                    <div className={s.painTags}>
                        {top.map((item, i) => (
                            <span key={`${item.keyword}-${i}`} className={s.painTag}>
                                {item.keyword}
                            </span>
                        ))}
                    </div>
            ...
            <div className={s.painSection}>
                <p className={s.painTitle}>개선 제안 TOP 3</p>
...
export default function B2BDashboard() {
    ...
    const topAge = data?.age_distribution?.reduce((a, b) => (b.pct > a.pct ? b : a), { label: '-', pct: 0 });
                        ...
                        <>
                            ...
                            <div className={s.metricsGrid}>
                                ...
                                <MetricCard
                                    ...
                                    sub={topBrand ? `점유율 ${topBrand.pct}%` : '—'}
                                    colorKey="purple"
                                />
                                <MetricCard
                                    title="주요 소비층"
                                    value={topAge?.label ?? '-'}
                                    sub={topAge?.pct ? `관심도 ${topAge.pct}%` : '—'}
                                    colorKey="blue"
                                />
                            </div>
                            ...
                            <div className={s.bottomRow}>
                                <div className={s.kwCard}>
                                    <h2 className={s.cardTitle}>Trend Drivers</h2>
                                    <p className={s.cardSub}>상승·시즌성·하락 키워드 분석</p>
                                    <TrendDrivers drivers={data.trend_drivers} />
                                </div>

                                <div className={s.painCard}>
                                    <h2 className={s.cardTitle}>Consumer Pain Points</h2>
                                    <p className={s.cardSub}>불만 키워드 · 개선 제안</p>
                            ...
                            <div className={s.reportCard}>
                                <div className={s.reportHeader}>
                                    <h2 className={s.reportTitle}>B2B 시장 요약</h2>
                                ...
                                <div className={s.strategyBox}>
                                    ...
                                    <span>
                                        {topBrand?.brand ?? category} 중심의 경쟁 구도를 확인하고,{' '}
                                        {topAge?.label ?? '주요 소비층'} 타겟 제품 전략을 강화하세요.
                                    </span>
                                </div>
                            </div>
                        </>
